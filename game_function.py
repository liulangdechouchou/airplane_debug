...
def check_keyup_events(event,ship,stats,ai_settings):
    """响应松开键盘"""
    if event.key == pygame.K_RIGHT:
        ...
        # write_high_score()
        record_highest_score(stats,ai_settings)
        sys.exit()



def record_highest_score(stats,ai_settings):
    """记录最高分到文件"""
    filename = ai_settings.filename
    username = getpass.getuser()
    high_score = {}
    with open (filename,'w') as file_object:
        high_score[username] =str(stats.high_score)
        json.dump(high_score,file_object)

def GetHistoryHighestScore(ai_settings):
    """ 读取最高分的文件"""
    filename = ai_settings.filename
    username = getpass.getuser()
    if os.path.exists(filename):
        with open (filename) as file_object:
            high_score_dict = json.load(file_object)
        high_score = int(high_score_dict[username])
        return high_score
    else:
        return 0
...
def check_events(ai_settings,screen,ship,bullets,stats,play_button,aliens,sb):
    ...
    for event in pygame.event.get():
        if event.type == pygame.QUIT:
            ...
            check_keyup_events(event,ship,stats,ai_settings)
